...
export function invitationEmail({
  ...
  const subject = `You're invited to ${businessName}'s Serenium portal`;
  const hello = fullName ? fullName.split(' ')[0] : 'there';
  const who = inviterName ? `${inviterName} at Serenium` : 'Your Serenium team';
  const html = shell(`
    <p style="font-size: 13px; color: ${orange}; font-weight: 600; text-transform: uppercase; letter-spacing: 0.18em; margin: 0 0 12px;">Invitation</p>
    <h1 style="font-size: 28px; font-weight: 900; letter-spacing: -0.025em; line-height: 1.15; margin: 0 0 16px;">Hey ${hello}, welcome to your portal.</h1>
    <p style="color: ${muted}; line-height: 1.6; margin: 0 0 4px;">${who} set up your Serenium client portal for <strong style="color: #fff;">${businessName}</strong>. It's where we'll collect everything we need to launch your campaigns — and once you're live, where your monthly reports will land.</p>
    <p style="color: ${muted}; line-height: 1.6; margin: 16px 0;">Click below to set up your account and get started. The link is valid for 14 days.</p>
    ${button(acceptUrl, 'Set up your account')}
    <p style="color: rgba(255,255,255,0.4); font-size: 12px; margin: 24px 0 0;">Or paste this into your browser:<br /><span style="color: ${muted}; word-break: break-all;">${acceptUrl}</span></p>
  `, `Set up your Serenium client portal for ${businessName}`);
  return { subject, html };
}
